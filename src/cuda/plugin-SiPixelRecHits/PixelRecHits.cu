...
namespace pixelgpudetails {
  ...
  TrackingRecHit2DCUDA PixelRecHitGPUKernel::makeHitsAsync(SiPixelDigisCUDA const& digis_d,
    ...
    TrackingRecHit2DCUDA hits_d(nHits, cpeParams, clusters_d.view().clusModuleStart(), stream);
    ...
    if (blocks)  // protect from empty events
      gpuPixelRecHits::getHits<<<blocks, threadsPerBlock, 0, stream>>>(
          cpeParams, bs_d.data(), digis_d.deviceView(), digis_d.hostView().nDigis(), clusters_d.view(), hits_d.view());
    ...
    if (nHits) {
      setHitsLayerStart<<<1, 32, 0, stream>>>(clusters_d.view().clusModuleStart(), cpeParams, hits_d.hitsLayerStart());
      cudaCheck(cudaGetLastError());
    }
